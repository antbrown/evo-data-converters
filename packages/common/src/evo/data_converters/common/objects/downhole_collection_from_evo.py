...
from evo_schemas.components.crs import Crs_V1_0_1, Crs_V1_0_1_EpsgCode, Crs_V1_0_1_OgcWkt
...
def get_crs_value(crs: Crs_V1_0_1) -> int | str:
    """Extract the CRS value as an EPSG code, WKT string, or 'unspecified'."""
    if isinstance(crs, Crs_V1_0_1_EpsgCode):
        return crs.epsg_code
    if isinstance(crs, Crs_V1_0_1_OgcWkt):
        return crs.ogc_wkt
    return crs
...
async def create_downhole_collection_from_evo(obj: DownloadedObject) -> DownholeCollection:
    ...
    dhc = DownholeCollection(
        collars=collars,
        coordinate_reference_system=get_crs_value(remote_dhc.coordinate_reference_system),
